                  ...
                  <div style={{ display:'grid', gridTemplateColumns:'repeat(5,1fr)', gap:8 }}>
                    {drawnCardsRef.current.map((card,i)=>(
                      <div key={i} style={{ display:'flex', flexDirection:'column', alignItems:'center', gap:3 }}>
                        <div style={{ position:'relative', width:'100%', aspectRatio:'2/3', borderRadius:8, overflow:'hidden', border:'1px solid rgba(138,155,224,0.4)', transform:card.reversed?'rotate(180deg)':'' }}>
                          <img src={card.img} alt={card.name} style={{ width:'100%', height:'100%', objectFit:'cover' }} />
                        </div>
                        <div style={{ fontSize:7, color:'#8a9be0', textAlign:'center' }}>{card.position}</div>
                      </div>
                    ))}
                  </div>
                </div>
