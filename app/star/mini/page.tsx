...
export default function MiniReading() {
  ...
  const handlePay = async () => {
    ...
    setPaying(true)
    try {
      // ① Stripe Payment Link にユーザー情報をクエリで渡す
      const theme = THEMES.find(t => t.id === selectedTheme)
      const params = new URLSearchParams({
        prefilled_email: email,
        client_reference_id: `${name}_${birth}_${selectedTheme}`,
      })
      // 決済後に結果ページへ
      const successUrl = `${window.location.origin}/star/result?name=${encodeURIComponent(name)}&birth=${encodeURIComponent(birth)}&theme=${encodeURIComponent(theme?.name || selectedTheme)}&paid=1&plan=mini`
      // Payment Linkは直接遷移（success_urlはStripeダッシュボードで設定）
      window.location.href = `${STRIPE_URL_500}?${params.toString()}`
    } catch (e: any) {
    ...
    <>
      <style>{`
        ...
        *{margin:0;padding:0;box-sizing:border-box;}
        ...
        /* ④ フォーム内緊急性 */
        .form-urgency{background:rgba(232,122,122,0.08);border:1px solid rgba(232,122,122,0.2);border-radius:8px;padding:10px 14px;text-align:center;margin-bottom:14px;}
        .form-urgency-text{color:#f5a0a0;font-size:12px;font-weight:600;}

        /* ⑤ ¥1,980導線（新規追加） */
        .mid-upsell{background:linear-gradient(135deg,rgba(100,60,140,0.2),rgba(60,30,100,0.15));border:1px solid rgba(155,89,182,0.35);border-radius:14px;padding:20px;text-align:center;margin-bottom:16px;}
        .mid-upsell-badge{display:inline-block;background:linear-gradient(135deg,#9b59b6,#6c3483);color:#fff;font-size:10px;font-weight:700;border-radius:20px;padding:3px 12px;letter-spacing:1px;margin-bottom:8px;}
        .mid-upsell-title{font-size:14px;color:var(--white);font-weight:600;margin-bottom:4px;}
        .mid-upsell-desc{font-size:12px;color:rgba(240,234,220,0.55);line-height:1.7;margin-bottom:10px;}
        .mid-upsell-price{font-family:'Cinzel',serif;font-size:20px;color:#c39bd3;margin-bottom:10px;}
        .mid-upsell-btn{display:inline-block;background:linear-gradient(135deg,#9b59b6,#6c3483);color:#fff;font-family:'Cinzel',serif;font-size:12px;font-weight:700;letter-spacing:1px;padding:10px 22px;border-radius:8px;text-decoration:none;transition:all 0.3s;}
        .mid-upsell-note{font-size:10px;color:rgba(240,234,220,0.35);margin-top:6px;}
      ...
      <div className="wrap">
        ...
        <div className="price-area">
          ...
          <div className="price-sub">即時表示 · 1テーマ集中リーディング · 🔒 Stripe安全決済</div>
        </div>

        {/* ② テーマ選択（変更後） */}
        <div className="theme-label">✦ 悩みのテーマを選んでください ✦</div>
        ...
        {selectedTheme && (
          <div style={{background:'rgba(201,168,76,0.08)',border:'1px solid rgba(201,168,76,0.25)',borderRadius:10,padding:'12px 16px',textAlign:'center',marginBottom:'16px'}}>
            <div style={{fontSize:'13px',color:'var(--gold2)',fontWeight:600,marginBottom:'4px'}}>
        ...
        <div className="card">
          ...
          <div className="form-urgency">
            <div className="form-urgency-text">⚡ 入力後すぐに決済→即時表示されます</div>
          </div>

          {/* ③ CTA変更「今の悩みの答えを知る（¥500）」 + ① Stripe接続 */}
          <button
            className="cta-btn"
            onClick={handlePay}
            disabled={paying}
          >
            {paying ? '決済ページへ移動中...' : '✦ 今の悩みの答えを知る（¥500）'}
          </button>
          <p className="cta-note">🔒 Stripe安全決済 · 即時表示 · 何度でも読み返せます</p>
        </div>

        {/* ⑤ ¥1,980導線（新規追加） */}
        <div className="mid-upsell">
          <div className="mid-upsell-badge">Standard Plan</div>
          <div className="mid-upsell-title">3テーマまとめて知りたい方へ</div>
          <div className="mid-upsell-desc">
            恋愛・仕事・金運を一度に深く鑑定。<br/>
            3ヶ月で起きる転機とタイムラインも確認できます。
          </div>
          <div className="mid-upsell-price">¥1,980</div>
          <a href="/star/chukan" className="mid-upsell-btn">✦ 3テーマをまとめて鑑定する</a>
          <div className="mid-upsell-note">¥500の3テーマ版＋総合メッセージ付き · 即時表示</div>
        </div>
        ...
        <p style={{textAlign:'center',fontSize:'13px',color:'rgba(240,234,220,0.4)',margin:'16px 0',lineHeight:1.8}}>
          ここまで見たあなたは、すでに答えに近づいています。<br/>
          次は、その答えを手に入れるだけです。
        </p>

        {/* ③ 下部CTA */}
        <button className="cta-btn" onClick={handlePay} disabled={paying}>
          {paying ? '移動中...' : '✦ 今の悩みの答えを知る（¥500）'}
        </button>
      ...
      <div className="fixed-cta">
        <div className="fixed-cta-text">
          ...
          <div className="fixed-cta-main">1テーマ集中リーディング · ¥500</div>
        </div>
        <button className="fixed-cta-btn" onClick={scrollToForm}>
          今すぐ鑑定
        </button>
      </div>
