...
<head>
  ...
  <style>
    ...
    .quiz-select{margin:5px 0;}
    .quiz-result{margin-top:10px;padding:10px;background:#fff;border-radius:8px;}
...
<body>
  ...
  <div class="container">
    ...
    <p>小学生の「つまずき」から、ぴったりのドリルと参考書をナビします。<br>
       例：「小1 くりあがり」「小1 ひらがな」「小3 漢字」などと入力してください。</p>

    <!-- 検索ボックス -->
    <div class="search-box">
      <input id="study-keyword" type="text" placeholder="例：小1 くりあがり / 小1 ひらがな / 小3 漢字">
      <button onclick="searchStudy()">ナビする</button>
    </div>
    ...
    <div class="fixed-block">
      ...
      <div class="book-item">
        <div class="book-title">百マス計算ドリル</div>
        <p class="small-note">毎日5〜10分で計算力の土台を作りたいときに。</p>
        <a class="amazon-link"
           href="https://www.amazon.co.jp/s?k=百マス計算+ドリル&tag=sincere1229-22"
           target="_blank">Amazonで見る</a>
      </div>
      <div class="book-item">
        <div class="book-title">九九ドリル</div>
        <p class="small-note">2年生以降、どの学年でも「暗記のやり直し」に使えます。</p>
        <a class="amazon-link"
    ...
    <div class="quiz-block">
      ...
      <p class="small-note">なんとなくで大丈夫です。普段の様子から一番近いものを選んでください。</p>

      <div class="quiz-question">
        <strong>Q1. どんな問題が得意そうですか？</strong><br>
      ...
      <div class="quiz-question">
        <strong>Q2. 一度に集中できる時間はどのくらいですか？</strong><br>
        <label class="quiz-select">
      ...
      <button onclick="runQuiz()" style="margin-top:10px;">タイプを診断する</button>
    ...
    <div class="columns">
      ...
      <div class="column-article">
        ...
        <p>
          小1でいちばん多いつまずきが「くりあがり・くりさがり」です。<br>
          10というまとまりのイメージがまだ弱いと、「9のつぎは10」「10を1と0に分ける」といった感覚がピンと来ません。<br>
          また、ノートのマスから数字がはみ出してしまう子は、そもそも「位」を意識するのが難しいことも多いです。
        </p>
        <p>
          まずは、10個のブロックを「まとめて1つのかたまり」にする遊びから始めたり、<br>
          「9＋1＝10」のパターンだけをくり返し練習するのがおすすめです。
        </p>
        <p class="small-note">
          おすすめ：くりあがりに特化した算数ドリルや、10のまとまりを絵で説明してくれる教材。<br>
      ...
      <div class="column-article">
        ...
        <p>
          ひらがな・カタカナは、「書けない＝遅れている」ではありません。<br>
          読む力のほうが先に育つことも多く、まずは「読める」ことをたくさんほめてあげることが大切です。
        </p>
        <p>
          書くときは、最初からきれいさを求めすぎないこと。<br>
          大きく・ゆっくり・なぞり書きから始めて、「自分で書けた！」という成功体験を積み重ねていきましょう。
        </p>
        <p class="small-note">
          おすすめ：なぞり書き中心のひらがなドリルや、音読しやすい短い絵本。<br>
      ...
      <div class="column-article">
        ...
        <p>
          子どもの勉強には、「コツコツ反復型」「ひらめき思考型」「マイペース集中型」など、いろいろなタイプがあります。<br>
          タイプに合わないやり方を無理に続けると、やる気そのものが落ちてしまいます。
        </p>
        <p>
          例えば、反復型の子には「同じドリルを毎日少しずつ」がおすすめ。<br>
          ひらめき型の子には、パズルや文章題など、少し考える問題を混ぜると楽しめます。<br>
          マイペース型の子には、「短い時間で終わる」「できたらすぐほめる」仕組みが向いています。
        </p>
        <p class="small-note">
          上の「勉強タイプ診断」でお子さんのタイプをチェックしてから、<br>
  ...
  <script>
    ...
    function searchStudy() {
      const key = document.getElementById('study-keyword').value.trim();
      const area = document.getElementById('study-result');
      if (!key) {
        area.innerHTML = "<p>「小1 くりあがり」「小1 ひらがな」「小3 漢字」などと入力してください。</p>";
        return;
      }
      ...
      list.forEach(item => {
        ...
        html += `
          <div class="book-item">
            ...
            <p>${item.reason}</p>
            <a class="amazon-link"
               href="https://www.amazon.co.jp/s?k=${encodeURIComponent(item.name)}&tag=sincere1229-22"
               target="_blank">Amazonで見る</a>
          </div>
        `;
      });
    ...
    function runQuiz() {
      ...
      const area = document.getElementById('quiz-result');

      if (!q1 || !q2 || !q3) {
        area.innerHTML = "<p>すべての質問にチェックを入れてください。</p>";
        return;
      }
      ...
      [q1.value, q2.value, q3.value].forEach(v => {
      ...
      if (repeatScore >= 2) {
        type = "コツコツ反復タイプ";
        message = "同じドリルを毎日少しずつ続けると、力がぐんぐん伸びるタイプです。百マス計算や、基礎問題が多いドリルと相性が良いです。";
      } else if (thinkScore >= 2) {
        type = "ひらめき思考タイプ";
        message = "パズルやなぞなぞなど、少し考える問題でやる気が出るタイプです。文章題や思考力ドリルを混ぜると、楽しく学べます。";
      } else {
        type = "マイペース集中タイプ";
        message = "短い時間にギュッと集中できるタイプです。10分で終わるドリルを用意して、「できた！」を積み重ねると続きやすくなります。";
      }

      area.innerHTML = `<p><strong>診断結果：${type}</strong></p><p>${message}</p>`;
    }
  </script>
</body>
</html>
