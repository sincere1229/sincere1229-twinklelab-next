...
【重要な出力ルール】
1. 一般論を書かない。ユーザー固有の文脈で書く
2. 「〜かもしれません」を減らし「〜の状態です」「〜の傾向があります」を使う
3. ${name}様という名前を3回以上使う
4. 「あなたの場合」という表現を積極的に使う
5. 最近ありそうな出来事や心の状態を具体的に言語化する
6. タロット3枚は必ずストーリーとして統合する（過去→原因、現在→本質、未来→変化）
7. 各セクションの最後に「今すぐできること」を2〜3個具体的に書く
8. 冒頭に必ず入れる：「まず最初にお伝えします。今の${name}様は人生の重要な転換点にいます。」
