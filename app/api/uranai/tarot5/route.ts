...
export async function POST(req: NextRequest) {
  try {
    ...
    const freeSchema =
      `{"theme":"今のテーマを表す短いフレーズ・15字以内","cardReads":["過去の見立て1行・30字以内","現在の見立て1行・30字以内","未来の見立て1行・核心を出さず「まだ見えていない流れが近づいているようです」程度","課題の見立て1行・核心を出さず余白を残す","アドバイスの見立て1行・核心を出さず余白を残す"],"overallTrend":"全体の傾向・90字程度・答えを断定せず、表面化していない流れがあると示唆して余白を残す","possibilityLine":"可能性を感じる一言・60字程度","hookMessage":"完全版へ誘導・90字程度。不安を煽らず期待で締める"}`

    const paidSchema =
      `{"cardMeanings":[{"position":"過去","name":"カード名","meaning":"このテーマでの詳しい意味・120字以上"},{"position":"現在","name":"","meaning":""},{"position":"未来","name":"","meaning":""},{"position":"課題","name":"","meaning":""},{"position":"アドバイス","name":"","meaning":""}],"forecast3months":"今後3か月の流れ・200字以上","flow30days":"今後30日の流れ・120字以上","flow90days":"今後90日の流れ・150字以上","halfYearGoal":"半年後の到達地点・120字以上","specialNote":"引いた5枚のカード構成から自然に導ける特別感の一言（例：同じテーマを強く示す配置／メッセージ性が強い展開など）。嘘や誇張は禁止。構成から言えることだけ・60字程度","actionGuide":"具体的な行動指針・180字以上","avoidActions":["避けるべき行動1","避けるべき行動2","避けるべき行動3"],"luckActions":["開運アクション1","開運アクション2","開運アクション3"],"roadmap":[{"period":"今すぐ〜1週間","action":"具体的な行動"},{"period":"1か月以内","action":"具体的な行動"},{"period":"3か月後（90日）","action":"具体的な行動"}],"themeDeepDive":"ご質問テーマ（恋愛・仕事・人間関係など）に沿った深掘り・200字以上","piyochanMessage":"安心の一言・100字程度","disclaimer":"この鑑定はAIによる傾向分析です。最終的な判断はあなたの気持ちを大切にしてください。"}`

    const systemPrompt = `あなたはタロットとホロスコープを統合して読み解く、やさしく寄り添う占い師です。
ルール：断定しない／不安を煽らない／恐怖訴求は禁止／最後は安心と前向きさで締める。
${mode === 'paid'
  ? '【有料版・¥1,980】「未来を買う商品」。情報量が多く、30日・90日・半年後の未来と、明日からの行動・時期・避けることまで具体的に。specialNote は引いた5枚の構成から自然に言える範囲のみ（嘘・誇張は禁止）。'
  : `【無料版】カード名と「ごく簡単な見立て」までにとどめる。
- 各カードの詳しい意味／具体的な未来予測／いつ動くべきか／何をすべきか／避けるべき行動／開運アクション／ロードマップ／詳細な行動指針は一切書かない（完全版の領域）。
- cardReads は引いた順（過去・現在・未来・課題・アドバイス）に1行ずつ。過去・現在は軽く触れてよいが、未来・課題・アドバイスは核心を出さず「まだ見えていない流れが近づいているようです」程度の余白で止める。
- overallTrend は答えを断定せず、「今は大きな選択の前」「まだ表面化していない重要な流れがある」のように、知りたくなる余白を残す。
- 無料で見せるのは「現状→傾向→可能性」まで。未来予測・行動指針・開運・ロードマップは完全版の領域。
- 最後は「もっと知りたい」で止める。`}
